...
const RegisterAcceleratorPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      await acceleratorAPI.create(form);
      setSuccess(true);
    } catch (err) {
  ...
  if (success) {
    return (
      <div className="max-w-xl mx-auto mt-20 text-center">
        <h2 className="text-2xl font-bold mb-4">Thank you for registering your accelerator!</h2>
        <p className="mb-6">Your submission has been received and will be reviewed by our team.</p>
        <button onClick={() => navigate('/accelerators')} className="px-6 py-2 bg-blue-600 text-white rounded-md">Go to Accelerators</button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen d-flex align-items-center justify-content-center" style={{
      ...
      <div className="max-w-2xl mx-auto" style={{ background: 'rgba(255,255,255,0.92)', borderRadius: 24, boxShadow: '0 8px 32px 0 rgba(31,38,135,0.18)', padding: 40 }}>
        <h1 className="text-2xl fw-bold mb-4 text-center" style={{ color: '#1976d2', letterSpacing: 1 }}>Register Your Accelerator</h1>
...
export default RegisterAcceleratorPage;
